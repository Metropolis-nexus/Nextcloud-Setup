...
- Allow people to mount external storage
  ...
  - FTP (This is extremely unreliable and can even let users accidentially crash the server, so only use it mount whatever needs to be mounted over FTP and disable it. The music app interestingly cannot load album covers over SFTP but works fine with FTP.)
